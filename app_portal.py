...
# تخصيص التصميم ليتناسب بسلاسة فائقة مع متصفحات الهواتف المحمولة وتوجيه الواجهة (RTL) وتنسيق الطباعة
st.markdown("""
    <style>
    body, html, [class*="css"] {
        direction: rtl;
    ...
    /* إعدادات مخصصة لطباعة أو تصدير PDF نظيف وخالٍ من عناصر التحكم */
    @media print {
        .stButton, .stTextInput, header, footer, [data-testid="stSidebar"], [data-testid="stDecoration"] {
    ...
    if student_id_input:
        ...
        with tab1:
            ...
            if not current_student_courses.empty:
                ...
                if st.button("🖨️ طباعة أو حفظ الجدول (PDF)", key="print_schedule_btn", use_container_width=True):
                    st.markdown("""
                        <script>
                            window.print();
                        </script>
        ...
        with tab2:
            ...
            if target_col_idx is not None:
                ...
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("🖨️ طباعة أو حفظ السجل (PDF)", key="print_transcript_btn", use_container_width=True):
                    st.markdown("""
                        <script>
                            window.print();
                        </script>
