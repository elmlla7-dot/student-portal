...
# تخصيص التصميم ليتناسب بسلاسة فائقة مع متصفحات الهواتف المحمولة وتوجيه الواجهة (RTL) وتنسيق الطباعة A4 Landscape
st.markdown("""
    <style>
    body, html, [class*="css"] {
        direction: rtl;
    ...
    /* تنسيق الجداول المباشرة لضمان اتجاه اليمين لليسار (RTL) */
    table {
        width: 100% !important;
    ...
    /* إعدادات الطباعة المباشرة على مقاس A4 Landscape وبدون عناصر تحكم */
    @media print {
        @page {
            size: A4 landscape;
            margin: 10mm;
    ...
    if student_id_input:
        ...
        with tab1:
            ...
            if not current_student_courses.empty:
                ...
                # دالة توليد جدول HTML متوافق مع RTL للجدول الحالي
                def style_html_schedule_table(df):
                    html = "<div style='overflow-x:auto;'><table dir='rtl'><thead><tr>"
                ...
                st.markdown(style_html_schedule_table(student_records_display), unsafe_allow_html=True)
        ...
        with tab2:
            ...
            if target_col_idx is not None:
                ...
                for idx, row in df_transcript.iterrows():
                    ...
                    # تنظيف وتصحيح عرض اسم الأستاذ إذا كان رقم أو فارغ
                ...
                # دالة توليد جدول HTML للسجل الدراسي مع ضمان اتجاه اليمين لليسار (RTL)
                ...
                # عرض جدول السجل بنظام HTML المباشر المتجه من اليمين لليسار
                st.markdown(style_html_table(df_result_transcript), unsafe_allow_html=True)
